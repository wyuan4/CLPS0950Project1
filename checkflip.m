...
%on any middle card. Replaces middle card with flippable card in hand, and
...
%players have jokers, puts down the joker, and then randomly assigns
%one of the players to put down the next random card (any card). continuing
%the game after that.
len = length(hand);

if len == 0 %if no cards in the hand, returns empty hand and same middle cards.
    new_hand=hand;
    new_mc1=mc1;
    new_mc2=mc2;
else
    %Goes through hand and replaces middle card if possible while removing 
    %card from hand. Also checks if middle card is a joker. Also checks if 
    %cards can crossover (ace to king, and vice versa)
    for i=randi(len):-1:1
